	...
	\begin{frame}{\insertsubsection}
		\begin{table}[htbp]
			...
			\begin{tabularx}{0.9\textwidth}{lccc}				
				& $E_\text{total}$ & $E_\text{bond}(\ce{O-NO})$ \footfullcite{ono}& $E_\text{int.}(\ce{NO})$ \\
				\toprule
				Peak 1 & \multirow{2}[2]{*}{$\num{43058.49}\unit{\per \centi \meter}$} & \multirow{8}[8]{*}{$\num{25128.57}\unit{\per \centi \meter}$} & \multirow{2}[2]{*}{\qquad\qquad\,\;\;$\num{5814.032698}\unit{\per \centi \meter}$\qquad\qquad\,\;\;} \\
				$\num{464.484}\unit{\nano\meter}$ & & & \\
				\cmidrule{1-2}\cmidrule{4-4}
				Peak 2 & \multirow{2}[2]{*}{$\num{43092.81}\unit{\per \centi \meter}$} & & \multirow{2}[2]{*}{$\num{5965.968317}\unit{\per \centi \meter}$} \\
				$\num{464.114}\unit{\nano\meter}$ & & & \\
				\cmidrule{1-2}\cmidrule{4-4}
				Peak 3 & \multirow{2}[2]{*}{$\num{43395.69}\unit{\per \centi \meter}$} & & \multirow{2}[2]{*}{$\num{6239.695895}\unit{\per \centi \meter}$} \\
				$\num{460.875}\unit{\nano\meter}$ & & & \\
				\cmidrule{1-2}\cmidrule{4-4}
				Peak 4 & \multirow{2}[2]{*}{$\num{43796.34}\unit{\per \centi \meter}$} & & \multirow{2}[2]{*}{$\num{8004.278786}\unit{\per \centi \meter}$} \\
				$\num{456.659}\unit{\nano\meter}$ & & & \\
				\bottomrule
			\end{tabularx}
		\end{table}
		\onslide<2>{\begin{block}{Rot. energy level\footfullcite{vj}}
			Simulated data from PGOPHER\footfullcite{pgopher}.
		\end{block}}
	\end{frame}
...
\begin{table}[htbp]
	...
	\begin{tabularx}{0.9\textwidth}{l|cc}
		...
		\toprule
		\multirow{2}[1]{*}{$^3 P_2$} & $11081.356\unit{\per \centi\meter}$ & $1923.847\unit{\per \centi\meter}$ \\
		& $10964.609\unit{\per \centi\meter}$ & $1903.578\unit{\per \centi\meter}$ \\
		\multirow{2}[1]{*}{($\num{0}\unit{\per \centi \meter}$)} & $10794.143\unit{\per \centi\meter}$ & $1873.983\unit{\per \centi\meter}$ \\
		& $9398.766\unit{\per \centi\meter}$ & $1631.730\unit{\per \centi\meter}$ \\
		\midrule
		\multirow{2}[1]{*}{$^3 P_1$} & $10922.731\unit{\per \centi\meter}$ & $1896.308\unit{\per \centi\meter}$ \\
		& $10805.984\unit{\per \centi\meter}$ & $1876.039\unit{\per \centi\meter}$ \\
		\multirow{2}[1]{*}{($\num{158.625}\unit{\per \centi \meter}$)} & $10635.518\unit{\per \centi\meter}$ & $1846.444\unit{\per \centi\meter}$ \\
		& $9240.141\unit{\per \centi\meter}$ & $1604.191\unit{\per \centi\meter}$ \\
		\midrule
		\multirow{2}[1]{*}{$^3 P_0$} & $10854.379\unit{\per \centi\meter}$ & $1884.441\unit{\per \centi\meter}$ \\
		& $10737.631538\unit{\per \centi\meter}$ & $1864.172\unit{\per \centi\meter}$ \\
		\multirow{2}[1]{*}{($\num{226.977}\unit{\per \centi \meter}$)} & $10567.166\unit{\per \centi\meter}$ & $1834.577\unit{\per \centi\meter}$ \\
		& $9171.789\unit{\per \centi\meter}$ & $1592.324\unit{\per \centi\meter}$ \\
		\bottomrule
	\end{tabularx}%
\end{table}%
\end{frame}
...
\begin{table}[htbp]  
	...
	\begin{tabular}{l|cc}    
		...
		\toprule    
		\multirow{2}[1]{*}{$^3 P_2$} & $1832.932\unit{\meter \per \second}$ & $253.177$ \\          
		& $1824.013\unit{\meter \per \second}$ & $253.650$ \\    
		\multirow{2}[1]{*}{($\num{0}\unit{\per \centi \meter}$)} & $1826.228\unit{\meter \per \second}$ & $256.147$ \\          
		& $1719.564\unit{\meter \per \second}$ & $246.776$ \\    
		\midrule    
...
\end{frame}
	\begin{frame}{\insertsubsection}{\insertsubsubsection}
		\begin{figure}[H]
			\centering
\begin{tikzpicture}
	\node at (0,0){\includegraphics[width = 0.9\textwidth]{3p012.eps}};
	\node at (-4,-2){{\footnotesize $\frac{d v}{d\, \text{px}} = 12.04\unit{\meter \per \second}/\text{px}\,@\, ^3P_2 $}};
	\node at (0,-2){{\footnotesize $\frac{d v}{d\, \text{px}} = 12.14\unit{\meter \per \second}/\text{px}\,@\, ^3P_1$}};
	\node at (4,-2){{\footnotesize $\frac{d v}{d\, \text{px}} = 12.18\unit{\meter \per \second}/\text{px}\,@\, ^3P_0$}};
\end{tikzpicture}
\end{figure}
\end{frame}
\begin{frame}{\insertsubsection}{\insertsubsubsection}
	\begin{figure}[H]
		...
		\node at (0,-3.3){$\frac{d v}{d\, \text{px}} = 12.12\unit{\meter \per \second}/\text{px}\,@\, \text{Average}$};
\end{tikzpicture}
		\end{figure}
	\end{frame}
%	
