...
\centering
	\begin{tikzpicture}
		...
		\onslide<2>{\node[below right] at (0.3,0){(0,0)};}
		\onslide<3->{
		\draw (0,0)--(4.2,4.2);
		}
		\onslide<4>{
		\draw[dotted,thick] (0,0) circle (0.3);
		\draw[<->,red] (0.35,0.1)--(4.45,4.1);
		}
		\onslide<5->{
		\filldraw[trigon] (4.45,4.45) circle (0.42);
		}
		\onslide<6->{
		\draw[cyan] (-0.7,-0.7)--(4.9,4.9) ;
		\node[above left,anchor=base east,cyan] at (2,2){Fitted $y=f\left( x\right) $};
		}
	\end{tikzpicture}
\end{figure}
\end{column}
\begin{column}{0.5\textwidth}
	\onslide<4->{If we assume a virtual zero point:
	\\
